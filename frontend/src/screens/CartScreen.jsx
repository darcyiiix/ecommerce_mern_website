            ...
            <div className='hidden max-sm:block flex items-center'>
                ...
                <td className="py-4">
                  <div className="flex items-center mx-4">
                    ...
                    <button className="mr-2" onClick={() => decrementQuantity(item._id)} onChange={(e) => addToCartHandler(item._id, Number(e.target.value))}>−</button>
                    <span className="text-center w-8">{quantities[item._id] || 1}</span>
                    <button className="ml-2" onClick={() => incrementQuantity(item._id, item.countInStock)}>+</button>
                  </div>
                </td>
              ...
              <td className="max-sm:hidden py-4">
                ...
                <td className="max-sm:hidden py-4 text-red-500">${(item.price * (quantities[item._id] || 1)).toFixed(2)} <a href="#" onClick={() => removeFromCartHandler(item._id)} className='block underline text-[12px] text-red-700'>Remove</a></td>
            </tr>
